...
@handle_errors(operation_name="filtered_web_search", return_dict=True)
...
@handle_errors(operation_name="content_web_search", return_dict=True)
def perform_content_search(query: str, content_type: str = "general", max_results: int = 5) -> Dict[str, Any]:
    ...
    # Enhance query based on content type (user-defined, no assumptions)
    enhanced_query = query.strip()
    
    # Apply user-specified content type as search context (no hardcoded assumptions)
    if content_type and content_type.lower() != "general":
        enhanced_query += f" {content_type}"
...
@handle_errors(operation_name="query_validation", return_dict=True)
...
def get_search_capabilities() -> Dict[str, Any]:
    ...
    return {
        ...
        "content_types": "user_defined_no_restrictions",
        "filters": [
            "domain_restriction",
            "date_range",
            "content_type",
