#!/usr/bin/env python3
"""
Fun Image Editor - Analyzes an image, finds funny text, and creates a meme
"""
...
import sfa_main  # Import to access the analyze_image function
...
async def analyze_image_with_claude():
    """Analyze the image using the Claude vision capabilities"""
    print("Analyzing the image...")
    
    try:
        # Call analyze_image from sfa_main
        # This is a bit hacky but works for testing purposes
        response = await sfa_main.handle_tool_call("analyze_image", {
            "image_path": IMAGE_PATH,
            "analysis_type": "detailed"
        })
        
        # Clean up the response (it's in JSON)
        import json
        try:
...
def get_funny_quotes(keyword="programmer humor", num_quotes=10):
    """Fetch some funny quotes/memes from the web based on keywords derived from image analysis"""
    print(f"Searching for funny quotes related to '{keyword}'...")
...
async def main():
    ...
    # Step 2: Get funny quotes
    quotes = get_funny_quotes()
    print("\nFound funny quotes:")
    for i, quote in enumerate(quotes[:5], 1):
        print(f"{i}. {quote}")
    
    # Step 3: Select a quote for the meme
