#!/bin/bash
# Install script for the standalone token counter
...
# Save the token script to bin directory
echo "Creating token command in ${BIN_DIR}..."
cat > "${BIN_DIR}/token" << 'EOF'
#!/usr/bin/env python3
"""
Self-contained token counter command-line tool
Usage:
    token "text in quotes"  - Count tokens in text
    token file.md           - Count tokens in file
    token directory/        - Count tokens in directory
"""
...
# Make the token command executable
chmod +x "${BIN_DIR}/token"

# Success message
echo "Token counter installed!"
echo "- 'token' command installed to ${BIN_DIR}"
echo ""
echo "You can now use the token command:"
echo "  token \"text to count\"  - Count tokens in text"
echo "  token file.md          - Count tokens in a file"
echo "  token directory/       - Count tokens in all files in directory"
echo ""
echo "If your old token command is still being used, you may need to:"
echo "  1. Close and reopen your terminal"
echo "  2. Run 'hash -r' to clear command cache"
echo "  3. Check your PATH to ensure ~/bin comes before other directories"