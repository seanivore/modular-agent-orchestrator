...
First the workflow is set up by invoking the `sfa` command:

```bash
sfa doc-content-plan-config.json
```
...
This workflow can be easily run in two steps.
...
Then the workflow can be run using the generated command:

```bash
doc-content-plan
```

Alternatively, you can execute the workflow directly:

```bash
sfa -e doc-content-plan-config.json
```
