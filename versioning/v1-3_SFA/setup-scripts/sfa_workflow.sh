...
if [ -z "$MODEL" ]; then
    MODEL=$(echo $CONFIG_INFO | python3 -c "import json, sys; print(json.load(sys.stdin).get('model', ''))")
fi
...
if [ -f "$README_PATH" ] && [ "$CREATE_DETAILED_README" = "false" ]; then
    ...
    # Generate README using the configured model (Requesty for Gemini, Anthropic for Claude)
    ...
    if [ "$CREATE_DETAILED_README" = "true" ] || [ ! -f "$README_PATH" ]; then
        TARGET_PATH=$([ "$CREATE_DETAILED_README" = "true" ] && echo "$DETAILED_README_PATH" || echo "$README_PATH")
            ...
            # Create README content using Requesty with the model from config
...
# Try to import openai for Requesty, fallback if not available
try:
    import openai
...
prompt_parts = [
    'Generate a README.md file for a Single-File Agent workflow based on this JSON configuration:',
    '',
    '```json',
    config_json,
    '```',
...
# Use Requesty API if model starts with google/ and OpenAI is available
if model_name.startswith('google/') and OPENAI_AVAILABLE:
    try:
        # Use Requesty with OpenAI SDK format
        ...
        # Fallback to a comprehensive README if API fails
        workflow_name = config.get('A', 'Workflow')
        use_case_dir = config.get('F', '')
        use_case_name = use_case_dir.split('/')[-2] if '/' in use_case_dir else 'workflow'
        
        readme_content = f"""# {workflow_name.title()} - Single-File Agent Workflow

This workflow helps with {use_case_name.replace('-', ' ')} tasks using AI automation.
...
"""
        ...
        workflow_key = next((k for k in config.keys() if k not in ['A', 'F', 'M']), None)
        if workflow_key and isinstance(config[workflow_key], dict):
            for i, (task_name, task_config) in enumerate(config[workflow_key].items(), 1):
                task_info = task_config[0] if isinstance(task_config, list) and len(task_config) > 0 else {}
                task_description = task_info.get('X', 'No description available')
                task_role = task_info.get('U', 'AI Assistant')
                
                readme_content += f"""### {i}. {task_name}
**Role**: {task_role}
...
print(f'README generated at {target_path} using {model_name}')
PYTHON_SCRIPT_END

            # Run the Python script with the model from config
            python3 /tmp/readme_generator.py "$CONFIG_FILE" "$TARGET_PATH" "$MODEL"
