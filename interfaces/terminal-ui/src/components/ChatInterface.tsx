...
export const ChatInterface: React.FC = () => {
  ...
  const { exit } = useApp();
  const pythonBridge = useRef<PythonBridge | null>(null);
  ...
  const handleMockSlashCommand = async (command: string) => {
    switch (command) {
      case 'help':
        setTimeout(() => {
  ...
  const handleMockConversation = async (userInput: string) => {
    setTimeout(() => {
  ...
  return (
    <Box flexDirection="column" height="100%" padding={1}>
      ...
      <Box marginTop={1}>
        ...
        <Text color={isConnected ? '#90ee90' : '#ff6b6b'}>
